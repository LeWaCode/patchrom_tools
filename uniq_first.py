...
def filter_lewa(lewa_file, filter_file, make_dir):
	lines = []
	lewa_apps = os.popen("make -C "+ make_dir + " lewa-apps-included").read()
	with open(lewa_file, 'r') as mf:
		lines = mf.readlines()

	with open(filter_file, 'w') as ff:
		for line in lines:
			if line.split():
				if (line.split()[0].split('"')[1] in lewa_apps):
					ff.write(line)
...
def main(args):
	...
	filter_lewa(args[1], filtered, args[2])
